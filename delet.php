					...
					<div class="rm-wrapper">
						...
						<div class="rm-middle">
								...
								$result = mysqli_query($con,"SELECT * FROM contenido WHERE IdOrden ='$_GET[ido]'");
								 $salida="<h4>Contenido de la Orden #$_GET[ido]</h4><dl>";
									while($row = mysqli_fetch_array($result)){
										...
										$salida.="<dt><a class='elimina' href='#' onclick='elimina($row[idAlimento])' >$nom[Nombre]</a></dt><dd>$row[cantidad] <input type='hidden' value='$row[idAlimento]' id='$row[idAlimento]'></dd>";
							...
							?>
								...
								<button onclick="window.location='deletcocina.php<?php echo '?ido='.$_GET['ido']; ?>'">Eliminar de Cocina</button>
									</div>
								</div><!-- /rm-content -->
