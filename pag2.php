					...
					<div class="rm-wrapper">
						...
						<div class="rm-cover">
							...
							<?php
								...
								$result = mysqli_query($con,"SELECT * FROM alimento WHERE IdSub ='5'");
								$sub= mysqli_query($con,"SELECT * FROM submenu WHERE idSub='5'");
						...
						<div class="rm-middle">
							...
							<?php
								$result = mysqli_query($con,"SELECT * FROM alimento WHERE IdSub ='6'");
								$sub= mysqli_query($con,"SELECT * FROM submenu WHERE idSub='6'");
								$rsub =  mysqli_fetch_array($sub);
								 $salida="<h4>$rsub[Descripcion]</h4><dl>";
									while($row = mysqli_fetch_array($result)){
										$salida.="<dt><a href='#' class='rm-viewdetails' data-thumb='images/11.jpg' >".nl2br(htmlentities($row['Nombre']))."</a></dt><dd><input type='hidden' value='$row[idAlimento]' id='".nl2br(htmlentities($row['Nombre']))."'></dd>";
